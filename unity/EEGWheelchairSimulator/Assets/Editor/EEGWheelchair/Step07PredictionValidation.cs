...
namespace EEGWheelchairSimulator.Editor
{
    ...
    [InitializeOnLoad]
    public static class Step07PredictionValidation
    {
        private const string ActiveKey = "EEG.Step07.ValidationActive";
        private const string FinishedKey = "EEG.Step07.ValidationFinished";
        ...
        static Step07PredictionValidation()
        {
        ...
        public static void Begin()
        {
            ...
            SessionState.SetFloat("EEG.Step07.Deadline", (float)EditorApplication.timeSinceStartup + 150f);
        ...
        private static void OnPlayMode(PlayModeStateChange state)
        {
            ...
            {
                SessionState.SetBool(FinishedKey, false);
                EditorApplication.Exit(SessionState.GetInt("EEG.Step07.Result", 1));
            }
        }
        ...
        private static void Tick()
        {
            ...
            try
            {
                if (EditorApplication.timeSinceStartup > SessionState.GetFloat("EEG.Step07.Deadline", 0f))
                    throw new TimeoutException("Step 7 Play Mode validation timed out.");
        ...
        private static void Finish(Exception error)
        {
            ...
            SessionState.SetInt("EEG.Step07.Result", error == null ? 0 : 1);
            if (error != null) Debug.LogException(error);
            else Debug.Log("STEP07_PLAY_OK: real Play Mode keyboard input, Input System pointer clicks, stop gate, reset, HUD/buttons and camera passed.");
        ...
        private static IEnumerator RunChecks()
        {
            ...
            foreach (var item in PredictionChecks(simulation, movement, hud)) yield return item;
            ...
            WheelchairMovement movement, WheelchairStatusUI hud, CameraFollow camera, Vector3 initialPosition, Quaternion initialRotation)
        {
            ...
            Debug.Log("STEP07_SOCKET_OK: actual Python sender; metadata; four commands; source/STOP/RESET gates; stale-command invalidation; disconnect/reconnect; split/combined/invalid/oversized NDJSON; camera/HUD; listener restart.");
        }
        ...
        private static System.Collections.Generic.IEnumerable<object> PredictionChecks(SimulationController simulation,
            WheelchairMovement movement, WheelchairStatusUI hud)
        {
            var receiver = movement.GetComponent<PythonWheelchairReceiver>();
            string Label(string name) => hud.transform.Find("StatusPanel/" + name).GetComponent<Text>().text;
            simulation.ResetSimulation();
            for (int i = 0; i < 3; i++) yield return null;
            Check(Label("ConnectionText") == "CONNECTION: WAITING" && Label("PredictionText") == "PREDICTION: -"
                && Label("ConfidenceText") == "CONFIDENCE: -", "Initial waiting HUD is stale.");
            Vector3 position = movement.transform.position;
            Quaternion rotation = movement.transform.rotation;
            string[] args = Environment.GetCommandLineArgs();
            int pythonIndex = Array.IndexOf(args, "-pythonExe");
            sender = new System.Diagnostics.Process
            {
                StartInfo = new System.Diagnostics.ProcessStartInfo(args[pythonIndex + 1],
                    "-u \"" + Path.GetFullPath("Tools/PythonTestSender/stream_sender.py") + "\" --interval 0.15")
                { UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true }
            };
            sender.Start(); sender.BeginOutputReadLine(); sender.BeginErrorReadLine();
            foreach (var item in WaitFor(() => receiver.LastSequence >= 8, "Continuous fake predictions were not received")) yield return item;
            CheckPose(movement.transform, position, rotation, "Streaming predictions moved stopped Cube.");
            Check(Label("ConnectionText") == "CONNECTION: CONNECTED" && receiver.HasPrediction, "Connected prediction HUD missing.");
            Check(Label("PredictionText") == "PREDICTION: " + receiver.LastReceivedCommand, "Prediction HUD is not last received packet.");
            Check(Label("ConfidenceText") == "CONFIDENCE: " + (receiver.LastConfidence * 100).ToString("F1", System.Globalization.CultureInfo.InvariantCulture) + "%",
                "Confidence HUD percentage mismatch.");
            Check(Label("MoveText") == "MOVE: STOPPED" && Label("SteeringText") == "STEERING: STRAIGHT", "Stopped streaming movement HUD mismatch.");
            long? previousSequence = receiver.LastSequence;
            simulation.StartSimulation();
            Check(receiver.CurrentCommand == "STOP" && receiver.LastSequence == previousSequence, "START replayed prediction or cleared display history.");
            foreach (var item in WaitFor(() => receiver.LastSequence >= previousSequence + 6, "Running stream stopped receiving")) yield return item;
            Check(Vector3.Distance(position, movement.transform.position) > 0.001f, "New running stream did not move Cube.");
            simulation.StopSimulation();
            position = movement.transform.position; rotation = movement.transform.rotation;
            previousSequence = receiver.LastSequence;
            foreach (var item in WaitFor(() => receiver.LastSequence >= previousSequence + 4, "STOP stopped prediction receive")) yield return item;
            CheckPose(movement.transform, position, rotation, "STOP failed while streaming.");
            simulation.StartSimulation();
            foreach (var item in WaitFor(() => receiver.CurrentCommand == "FORWARD", "No fresh FORWARD after restart")) yield return item;
            sender.Kill();
            foreach (var item in WaitFor(() => sender.HasExited && !receiver.IsConnected, "Stream termination did not disconnect")) yield return item;
            sender.Dispose(); sender = null;
            for (int i = 0; i < 3; i++) yield return null;
            Check(!receiver.HasPrediction && receiver.LastSequence == null && receiver.CurrentCommand == "STOP", "Disconnect retained prediction/command.");
            Check(Label("ConnectionText") == "CONNECTION: WAITING" && Label("PredictionText") == "PREDICTION: -"
                && Label("ConfidenceText") == "CONFIDENCE: -", "Disconnect HUD did not clear.");

            wire = new System.Net.Sockets.TcpClient();
            var connect = wire.ConnectAsync("127.0.0.1", 5055);
            foreach (var item in WaitFor(() => connect.IsCompleted && receiver.IsConnected, "Prediction peer reconnect failed")) yield return item;
            simulation.StopSimulation();
            int count = receiver.ReceivedCount;
            WriteWire("{\"command\":\"LEFT\",\"confidence\":0.914,\"timestamp\":1726362000.125,\"sequence\":12}\n");
            foreach (var item in WaitFor(() => receiver.ReceivedCount > count, "v0.2 message missing")) yield return item;
            for (int i = 0; i < 3; i++) yield return null;
            Check(receiver.LastSequence == 12 && receiver.LastTimestamp == 1726362000.125, "v0.2 sequence/timestamp lost.");
            Check(Label("PredictionText") == "PREDICTION: LEFT" && Label("ConfidenceText") == "CONFIDENCE: 91.4%"
                && Label("SteeringText") == "STEERING: STRAIGHT", "Received prediction was inferred from movement.");
            int previewIndex = Array.IndexOf(args, "-hudPreviewPath");
            if (previewIndex >= 0)
            {
                // Batch frames run much faster than the Button color fade (0.08 s).
                // Let the existing UI transition finish before capturing a stopped-state preview.
                double settled = EditorApplication.timeSinceStartup + 0.15;
                foreach (var item in WaitFor(() => EditorApplication.timeSinceStartup >= settled, "HUD fade did not settle")) yield return item;
                Type.GetType("EEGWheelchairSimulator.Editor.Step04HudValidation, Assembly-CSharp-Editor")
                    .GetMethod("RenderPreview", BindingFlags.Static | BindingFlags.NonPublic)
                    .Invoke(null, new object[] { hud.GetComponent<Canvas>(), args[previewIndex + 1] });
            }
            simulation.StartSimulation();
            position = movement.transform.position; rotation = movement.transform.rotation;
            for (int i = 0; i < 8; i++) yield return null;
            CheckPose(movement.transform, position, rotation, "Old prediction replayed on START.");
            Check(Label("PredictionText") == "PREDICTION: LEFT" && Label("SteeringText") == "STEERING: STRAIGHT", "START conflated prediction with movement.");
            count = receiver.ReceivedCount;
            WriteWire("{\"command\":\"RIGHT\",\"confidence\":0,\"timestamp\":2}\n");
            foreach (var item in WaitFor(() => receiver.ReceivedCount > count, "v0.1 message rejected")) yield return item;
            for (int i = 0; i < 4; i++) yield return null;
            Check(receiver.LastSequence == null && receiver.CurrentCommand == "RIGHT" && Label("ConfidenceText") == "CONFIDENCE: 0.0%",
                "v0.1/zero confidence compatibility failed (no threshold allowed).");
            count = receiver.ReceivedCount;
            WriteWire("{oops}\n{\"confidence\":0.8,\"timestamp\":1}\n{\"command\":\"LEFT\",\"timestamp\":1}\n"
                + "{\"command\":\"LEFT\",\"confidence\":-0.1,\"timestamp\":1}\n"
                + "{\"command\":\"LEFT\",\"confidence\":1.1,\"timestamp\":1}\n"
                + "{\"command\":\"UNKNOWN\",\"confidence\":0.8,\"timestamp\":1}\n\n  \r\n");
            for (int i = 0; i < 15; i++) yield return null;
            Check(receiver.IsConnected && receiver.ReceivedCount == count && receiver.CurrentCommand == "RIGHT"
                && Label("PredictionText") == "PREDICTION: RIGHT" && Label("ConfidenceText") == "CONFIDENCE: 0.0%",
                "Invalid message changed prediction or movement command.");
            WriteWire("{\"command\":\"STOP\",\"confidence\":1,\"timestamp\":3,\"sequence\":13}\n");
            foreach (var item in WaitFor(() => receiver.ReceivedCount > count, "Receiver stopped after invalid messages")) yield return item;
            for (int i = 0; i < 3; i++) yield return null;
            Check(Label("ConfidenceText") == "CONFIDENCE: 100.0%", "Confidence upper boundary formatting failed.");
            simulation.ResetSimulation();
            Check(!simulation.IsRunning && receiver.CurrentCommand == "STOP" && receiver.HasPrediction, "RESET did not separate prediction and motion.");
            simulation.SetControlSource(WheelchairControlSource.Keyboard);
            wire.Dispose(); wire = null;
            for (int i = 0; i < 3; i++) yield return null;
            Check(!receiver.HasPrediction && Label("ConnectionText") == "CONNECTION: -" && Label("PredictionText") == "PREDICTION: -"
                && Label("ConfidenceText") == "CONFIDENCE: -", "Keyboard mode retained Python HUD data.");
            Debug.Log("STEP07_PREDICTION_OK: real Python stream, STOPPED display/blocked movement, new-only START, disconnect/reconnect clearing, v0.1/v0.2, metadata, invalid messages and confidence boundaries.");
        }
